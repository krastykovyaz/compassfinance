...
/**
 * DeepSeek normally returns raw JSON in JSON mode, but a model can sometimes
 * wrap an otherwise valid object in ```json ... ``` despite the request.
 * Strip only that presentation wrapper; never attempt to "repair" arbitrary
 * malformed JSON because that could make invalid model output look trusted.
 */
