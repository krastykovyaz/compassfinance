...
import { getHyperliquidCoinForAsset } from "./asset-mapping";
...
export type AssetTradingProfile = {
  ...
  underlying: string;
  /** Technical Hyperliquid coin symbol (e.g. "BTC"), or null when no
   * verified Hyperliquid perpetual market exists for this asset. Hidden
   * from every primary UI surface — shown only inside expandable Details
   * (see AssetDetailsPanel), never as a primary label. */
